...
admin.get('/', async (c) => {
  ...
  const stats = [
    { icon: 'fa-calendar-check', label: '활동 내역',   value: actCount?.c    ?? 0, iconColor: 'var(--bnk-dark-red)', bg: '#FFF0F0', link: '/admin/activities' },
    { icon: 'fa-pen-alt',        label: '블로그 포스트',value: postCount?.c   ?? 0, iconColor: 'var(--bnk-gold)',     bg: '#FFF8EF', link: '/admin/posts' },
    { icon: 'fa-users',          label: '팀 멤버',      value: memberCount?.c ?? 0, iconColor: 'var(--bnk-dark-gray)', bg: '#F5F2EF', link: '/admin/members' },
    { icon: 'fa-envelope',       label: '새 문의',      value: newContacts?.c ?? 0, iconColor: 'var(--bnk-red)',      bg: '#FFF0F0', link: '/admin/contacts' },
  ]
      ...
      ${stats.map(s => `
        <a href="${s.link}" class="bg-white rounded-2xl p-5 card-hover shadow-sm border border-gray-100 flex items-center gap-4">
          <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <i class="fas ${s.icon} text-xl" style="color:${s.iconColor}"></i>
          </div>
          <div>
            <div class="text-2xl font-black" style="color:var(--bnk-dark-gray)">${s.value}</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      <div class="rounded-2xl p-6 text-white flex items-center gap-4"
        ...
        <div>
          ...
          <div class="text-sm" style="color:rgba(255,220,200,0.85)">누적 봉사 수혜자</div>
        </div>
      </div>
      <div class="rounded-2xl p-6 text-white flex items-center gap-4"
        style="background:linear-gradient(135deg,#5C3D20,#896E4A)">
        <div class="w-14 h-14 bg-white bg-opacity-20 rounded-xl flex items-center justify-center">
          <i class="fas fa-clock text-white text-2xl"></i>
        </div>
        <div>
          <div class="text-3xl font-black">${(totalHours?.s ?? 0).toLocaleString()}시간</div>
          <div class="text-sm" style="color:rgba(255,230,190,0.85)">누적 봉사 시간</div>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <a href="/admin/activities" class="text-xs font-medium transition-colors" style="color:var(--bnk-red)">전체 보기 →</a>
        </div>
        <div class="divide-y divide-gray-50">
          ${(recentActs.results as any[]).map(a => `
            <div class="px-6 py-3 flex items-center justify-between">
              ...
              <a href="/admin/activities/${a.id}/edit" class="text-xs font-medium transition-colors" style="color:var(--bnk-dark-red)">편집</a>
            </div>`).join('')}
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <a href="/admin/contacts" class="text-xs font-medium transition-colors" style="color:var(--bnk-red)">전체 보기 →</a>
        </div>
        <div class="divide-y divide-gray-50">
...
const categoryColors: Record<string, string> = {
  '디지털 교육': 'red', 'IT 장비 지원': 'gold', '보안 교육': 'gray', '기타': 'gray'
}
              ...
              <td class="px-5 py-3.5 text-right">
                <div class="flex items-center justify-end gap-2">
                  <a href="/admin/activities/${a.id}/edit"
                    class="tag-red px-3 py-1.5 rounded-lg text-xs font-medium transition-colors">
        ...
        <form action="${action}" method="POST" data-ajax data-method="${method}" class="space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" name="date" value="${values.date || ''}" placeholder="예: 2024.11.20" required
                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:outline-none"/>
            </div>
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-1.5">카테고리 *</label>
              <select name="category" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:outline-none">
          ...
          <div>
            ...
            <textarea name="description" rows="5" placeholder="활동 내용을 자세히 적어주세요"
              class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:outline-none resize-none">${values.description || ''}</textarea>
          </div>
        ...
        <tbody class="divide-y divide-gray-50">
          ${(rows.results as any[]).map(p => `
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-5 py-3.5 text-right">
                <div class="flex items-center justify-end gap-2">
                  <a href="/admin/posts/${p.id}/edit" class="tag-red px-3 py-1.5 rounded-lg text-xs font-medium"><i class="fas fa-edit mr-1"></i>편집</a>
        ...
        <form action="${action}" method="POST" data-ajax data-method="${method}" class="space-y-5">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div>
              ...
              <select name="tag" required class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:outline-none">
          ...
          <div>
            ...
            <textarea name="content" rows="10" required placeholder="포스트 내용을 입력하세요"
              class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:outline-none resize-none">${values.content || ''}</textarea>
          </div>
          <div class="flex items-center gap-2">
            <input type="checkbox" name="published" id="pub" value="1" ${values.published !== 0 ? 'checked' : ''} class="w-4 h-4 rounded"/>
            <label for="pub" class="text-sm text-gray-700 font-medium">공개 여부</label>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden card-hover">
            ...
            <div class="p-4">
              ...
              <div class="flex flex-wrap gap-1 mb-3">
                ${skills.map((s: string) => `<span class="tag-red text-xs px-2 py-0.5 rounded-full">${s}</span>`).join('')}
              </div>
              <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                <span class="text-gray-400 text-xs">봉사 ${m.years}년</span>
                <div class="flex gap-2">
                  <a href="/admin/members/${m.id}/edit" class="tag-gold px-3 py-1 rounded-lg text-xs font-medium">
                    <i class="fas fa-edit mr-1"></i>편집
                  </a>
        ...
        <form action="${action}" method="POST" data-ajax data-method="${method}" class="space-y-5">
          ...
          <div class="grid grid-cols-2 gap-5">
            ...
            <div>
              ...
              <select name="color" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:outline-none">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-5">
            ...
            ${statusOptions.map(s => `
              <button onclick="updateContactStatus(${ct.id}, '${s.value}')"
                class="px-4 py-2 rounded-xl text-xs font-bold border-2 transition-all
                  ${ct.status === s.value ? 'border-red-400' : 'border-gray-200 text-gray-500 hover:border-gray-400'}">
                ${s.label}
              </button>`).join('')}
          </div>
        </div>
        <div>
          ...
          <textarea id="memoText" rows="3" placeholder="내부 처리 메모 (공개되지 않습니다)"
            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:outline-none resize-none">${ct.memo || ''}</textarea>
